...
module ShopifyAPI
  ...
  class Session
    ...
    def initialize(url, token = nil, params = nil)
      ...
      ShopifyAPI::Base.site = site if url
    end
    ...
    private
      ...
      def self.validate_signature(params)
        ...
        sorted_params = params.except(:signature, :action, :controller, :locale).collect{|k,v|"#{k}=#{v}"}.sort.join
  ...
  class Base < ActiveResource::Base
    ...
    def self.authorize(url, token)
      self.site "#{ShopifyAPI::Session.protocol}://#{ShopifyAPI::Session.api_key}:#{Digest::MD5.hexdigest(ShopifyAPI::Session.secret + token)}@#{url}/admin"
    end
